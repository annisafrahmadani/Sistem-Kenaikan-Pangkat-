...
if (isset($_GET['add'])) {
    ...
    if (in_array($tipe, $ekstensi) === true) {
        if ($ukuranFile < 1048576) { //1 mb
            ...
            create("INSERT INTO pegawai VALUES ('$nip','$nama_pegawai','$foto','$tempat_lahir','$tgl_lahir','$jk','$no_hp','$agama','$email','$alamat','$goldarah','$stat_nikah','$j_jabatan','$b_bidang','aktif')");
    ...
    if ($_FILES['foto']['name'] == '') {
        $query = mysqli_query($conn, "UPDATE pegawai SET nip='$nip',nama_pegawai='$nama_pegawai', tempat_lahir='$tempat_lahir', tanggal_lahir='$tgl_lahir', jenis_kelamin='$jk', no_hp='$no_hp', agama='$agama', email='$email',alamat='$alamat',gol_darah='$goldarah',status_pernikahan='$stat_nikah', bidang='$b_bidang', jabatan='$j_jabatan', status_user='$stat_user' WHERE nip='$nipAsli' ");
        if ($query) {
            echo '<script>
        ...
        if (in_array($tipe, $ekstensi) === true) {
            if ($ukuranFile < 1048576) { //1 mb
                ...
                mysqli_query($conn, "UPDATE pegawai SET nip='$nip',nama_pegawai='$nama_pegawai', foto_pegawai='$foto' ,tempat_lahir='$tempat_lahir', tanggal_lahir='$tgl_lahir', jenis_kelamin='$jk', no_hp='$no_hp', agama='$agama', email='$email',alamat='$alamat',gol_darah='$goldarah',status_pernikahan='$stat_nikah', bidang='$b_bidang', jabatan='$j_jabatan', status_user='$stat_user' WHERE nip='$nipAsli' ");
                $lokasi = "../assets/img/" . $foto;
                $upload = move_uploaded_file($sumber, $lokasi);
                if ($upload) {
                    echo '<script>
